...
(function (EnabledFlags) {
    ...
    EnabledFlags[EnabledFlags["AllEnabled"] = 3] = "AllEnabled";
...
class Config {
    ...
    get isExtensionEnabled() {
        const featureIsEnabledResults = [this.isSbxEnabled(), this.isIpEnabled()];
...
class SBXFeatureManager {
    constructor(configNotifier, errorHandler, cache) {
        this.cache = cache;
...
class ConfigUpdater {
    constructor(configNotifier, messageSender, messageRouter, handshaker, onConfiguredRaiser, cache) {
        ...
        messageRouter.registerManyMessageHandler([MessageType.configChangedV3, MessageType.configChangedV4,
            ...
            MessageType.configChangedV24], message => {
            this.onConfigUpdated(message);
        });
    ...
    setConfig(configChanged) {
        ...
        if ("identityProtection" in configChanged) {
            ...
            if (isSerializedIPConfigV22(ipConfig)) {
...
class TabStalker {
    constructor() {
        ...
        this.tabListenerManager.addEvent(chrome.tabs.onCreated, this.onTabCreated.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onUpdated, this.onTabUpdated.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onMoved, this.onTabMoved.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onActivated, this.onTabActivated.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onHighlighted, this.onTabHighlighted.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onDetached, this.onTabDetached.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onAttached, this.onTabAttached.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onRemoved, this.onTabRemoved.bind(this));
        this.tabListenerManager.addEvent(chrome.tabs.onReplaced, this.onTabReplaced.bind(this));
        this.windowListenerManager.addEvent(chrome.windows.onCreated, this.onWindowCreated.bind(this));
        this.windowListenerManager.addEvent(chrome.windows.onRemoved, this.onWindowRemoved.bind(this));
        this.windowListenerManager.addEvent(chrome.windows.onFocusChanged, this.onWindowFocusChanged.bind(this));
    }
...
;// CONCATENATED MODULE: ../lib/host/identity-protection-report-types.ts
...
var IPRecordActivityType;
(function (IPRecordActivityType) {
    IPRecordActivityType["DroppedActivities"] = "droppedActivities";
    IPRecordActivityType["Navigation"] = "navigation";
    IPRecordActivityType["Redirect"] = "redirect";
    IPRecordActivityType["SbxLinkProtectionEvent"] = "sbxEvent";
})(IPRecordActivityType || (IPRecordActivityType = {}));
...
;// CONCATENATED MODULE: ./identity-protection-report-helper.ts
...
class IdentityProtectionReportHelper {
    constructor(phishingCategoryTracker, tabArchivist, hostHelperMessageSender) {
        this.phishingCategoryTracker = phishingCategoryTracker;
    ...
    navTypeToActivityType(navType) {
        switch (navType) {
            case NavType.navigation:
                return IPRecordActivityType.Navigation;
            case NavType.redirect:
                return IPRecordActivityType.Redirect;
            default:
                return undefined;
        }
    }
    ...
    makeDroppedActivitiesRecord(droppedCount, firstDroppedActivity) {
        return {
            activity: IPRecordActivityType.DroppedActivities,
            droppedCount,
    ...
    makeLinkProtectionRecord(event) {
        var _a, _b;
        return {
            activity: IPRecordActivityType.SbxLinkProtectionEvent,
    ...
    sendPreparepPhishingReport(tabId, type, urlSpec, navSeq, screenshot, categoryDetails, linkProtectionEvent) {
        ...
        const message = new PreparePhishingReportV22(tabId, Date.now(), this.extensionId, this.extensionVersion, this.cloudListVersion, type, urlSpec, screenshot, activities, annotations);
        this.hostHelperMessageSender.sendMessage(MessageType.preparePhishingReportV22, message);
    }
    prepareCredentialPhishingReport(tabId, urlSpec, screenshot, categoryDetails, linkProtectionEvent) {
        ...
        this.sendPreparepPhishingReport(tabId, IPReportType.CrendentialExtension, urlSpec, navSeq, screenshot, categoryDetails, linkProtectionEvent);
        return true;
    }
    sendSmartScreenReport(tabId, urlSpec) {
        var _a;
        ...
        this.sendPreparedReport(tabId);
    }
    sendPreparedReport(tabId) {
...
class WebRequestListener {
    constructor(configNotifier, tabStalker, placementManager, blockedPageController, fileBlocker, browserLauncher, phishingNavSequenceDetector, externalAppLinkController, featureManager, onConfigured, linkProtectionService) {
        ...
        this.otherListenersManager.addEvent(chrome.webRequest.onBeforeRedirect, this.onBeforeRedirect.bind(this), filter);
        this.otherListenersManager.addEvent(chrome.webRequest.onCompleted, this.onCompleted.bind(this), filter);
        this.otherListenersManager.addEvent(chrome.webRequest.onErrorOccurred, this.onErrorOccurred.bind(this), filter);
        this.headersListenerManager.addEvent(chrome.webRequest.onHeadersReceived, this.onHeadersReceived.bind(this), filter, ['blocking', 'responseHeaders']);
        this.blockingBeforeRequestListenerManager.addEvent(chrome.webRequest.onBeforeRequest, this.onBeforeRequest.bind(this), filter, ['blocking']);
        this.beforeRequestListenerManager.addEvent(chrome.webRequest.onBeforeRequest, this.onBeforeRequest.bind(this), filter);
    }
    onConfigChanged(config) {
    ...
    createLaunchedPageURL(launchedURL, contentType) {
        return this.browserLauncher.createLaunchedPageURL(launchedURL, contentType, true);
    }
    launchBrowser(url) {
        if (maybe_none(url)) {
            return;
    ...
    launchBrowserAndCloseTabWithContentType(tabId, launchURL, contentType, backToSrcURL = undefined) {
        ...
        const redirectURL = this.createLaunchedPageURL(launchURL, contentType);
        ...
        if (shouldRedirectAndKeepTab) {
            log_log(`Instead of cancelling the navigation and closing the tab we are ` +
    ...
    updateListeners() {
        let noListenersRegistered = true;
        const updateListener = (listenerManager, shouldRegister) => {
            if (shouldRegister) {
                listenerManager.registerListeners();
                noListenersRegistered = false;
            }
            else {
        ...
        const credentialProtectionEnabled = this.credentialProtectionEnabled;
        updateListener(this.otherListenersManager, shouldRegisterListeners && (blockingProtectionEnabled || credentialProtectionEnabled));
        updateListener(this.headersListenerManager, shouldRegisterListeners && isPDFProtectionEnabled);
        updateListener(this.blockingBeforeRequestListenerManager, shouldRegisterListeners && blockingProtectionEnabled);
        updateListener(this.beforeRequestListenerManager, shouldRegisterListeners && !blockingProtectionEnabled && credentialProtectionEnabled);
        if (noListenersRegistered) {
            this.clearState();
        }
        this.hasRegisteredListeners = !noListenersRegistered;
    }
    registerListeners() {
...
class DownloadTracker {
    constructor(messageSender, featureManager) {
        ...
        this.listenerManager.addEvent(chrome.downloads.onChanged, this.onChanged.bind(this));
    }
...
class TabSnapper {
    ...
    maybeTakeDelayedScreenshot() {
        if (!this.frozen && this.waitingForActive) {
            this.clearPendingSnap();
            this.snap();
        }
    }
    ...
    snap() {
        chrome.tabs.get(this.tabId, tab => {
            this.waitingForActive = false;
            if (tab.active) {
                chrome.tabs.captureVisibleTab(tab.windowId, screenshotData => this.screenshotData = screenshotData);
            }
            else {
...
class ContentScriptTabController {
    constructor(tabId, enabled, identityProtectionConfig, categoryTracker, onConfigured, hostHelperMessageSender, reportHelper, badgeManager) {
        this.tabId = tabId;
        this.enabled = enabled;
        ...
        this.updateConfig(identityProtectionConfig);
    }
    addFrameController(frameId, port) {
        if (frameId === TopLevelFrameId) {
        ...
        frameController.sendMessage(MessageType.frameLoadResponseV22, new FrameLoadResponseV22(frameId, this.linkProtectionWouldBlock, this.enabled, this.contentScriptConfig));
        this.frameControllers.add(frameController);
    }
    onChange(enabled, identityProtectionConfig) {
        this.enabled = enabled;
        this.updateConfig(identityProtectionConfig);
        this.onDataUpdated();
    }
    ...
    onDataUpdated() {
        this.sendMessageToAllFrames(MessageType.contentScriptDataV22, new ContentScriptDataV22(this.linkProtectionWouldBlock, this.enabled, this.contentScriptConfig));
    }
    ...
    onTopLevelFrameDisconnected() {
        if (this.reportCreated) {
            this.reportHelper.sendPreparedReport(this.tabId);
        }
        this.resetState();
    }
        ...
        switch (message.type) {
            ...
            case MessageType.onFrameDomUpdateV22:
                ...
                if (maybe_none(this.category) && pageHasInterestingInput) {
                    ...
                    controller.sendMessage(MessageType.onPhishingCategoryChangedV22, { category: this.category });
                }
                if (this.takeScreenshot) {
                    const pageHasInterestingCategory = maybe_some(this.category) && !isGoodCategory(this.category);
                    if (pageHasInterestingInput && pageHasInterestingCategory && !this.tripped && !this.suppressed) {
                        this.tabSnapper.takeScreenshot();
                    }
            ...
            case MessageType.freezeScreenshotV22:
                ...
                break;
            case MessageType.onPhishingOverlayClickV22:
                this.onOverlayClick(message.payload);
                break;
        }
    }
    onOverlayClick(message) {
        log_log(`ContentScriptTabController: overlay action ${IPOverlayAction[message.action]} for tab ${this.tabId}`);
        switch (message.action) {
            case IPOverlayAction.OverlayNotShown:
                this.addPhishingReportAction(IPReportActions.LearningModeBypass);
                break;
            case IPOverlayAction.BackToSafety:
                this.replaceTab();
                break;
            case IPOverlayAction.ViewWithInputDisabled:
                break;
            case IPOverlayAction.EnableInput:
                if (isInputAllowableOpMode(this.operationMode) && this.contentScriptConfig.allowUserToEnableInput) {
                    this.addPhishingReportAction(IPReportActions.InputAllowed);
                    this.sendMessageToAllFrames(MessageType.onPhishingOverlayClickV22, message);
                    this.badgeManager.setTabBadge(this.tabId, Colour.green);
                    if (this.category !== IPPageCategory.Blocked) {
                        if (maybe_some(this.url)) {
    ...
    onCategorised(details, urlSpec) {
        ...
        if (((_a = this.url) === null || _a === void 0 ? void 0 : _a.href) === urlSpec && !details.equals(this.categoryDetails)) {
            ...
            this.sendMessageToAllFrames(MessageType.onPhishingCategoryChangedV22, { category: details.category });
    ...
    preparePhishingReport() {
        ...
        let screenshot;
        if (this.takeScreenshot) {
            screenshot = this.tabSnapper.screenshot;
            if (maybe_none(screenshot)) {
                logError("Failed to prepare a phishing report because there is no screenshot");
                return;
            }
        }
        if (this.reportHelper.prepareCredentialPhishingReport(this.tabId, urlSpec, screenshot, this.categoryDetails, this.linkProtectionBlockEvent)) {
    ...
    get operationMode() {
        return this.contentScriptConfig.operationMode;
    }
    get category() {
        var _a;
...
class ContentScriptController {
    constructor(configNotifier, errorHandler, navigationNotifier, handshaker, linkProtectionService, tabNotifier, categoryTracker, onConfigured, hostHelperMessageSender, reportHelper, badgeManager) {
        this.categoryTracker = categoryTracker;
        this.onConfigured = onConfigured;
        this.hostHelperMessageSender = hostHelperMessageSender;
        this.reportHelper = reportHelper;
    ...
    onChange() {
        ...
        for (const controller of this.tabControllers.values()) {
            controller.onChange(enabled, this.identityProtectionConfig);
        }
        if (isMessageTypeSupported(MessageType.identityProtectionRunningV24, this.protocolVersion)) {
            this.hostHelperMessageSender.sendMessage(MessageType.identityProtectionRunningV24, { running: enabled });
        }
    }
    onTabActivated(tabId) {
        var _a;
    ...
    onUnresolvedPortConnected(port) {
        ...
        if (maybe_none(tabController)) {
            tabController = new ContentScriptTabController(tabId, this.enabled, this.identityProtectionConfig, this.categoryTracker, this.onConfigured, this.hostHelperMessageSender, this.reportHelper, this.badgeManager);
...
class BrowserSmartScreenTracker {
    constructor(configNotifier, tabUpdatedNotifier, reportHelper) {
        this.reportHelper = reportHelper;
    ...
    onTabUpdated(tabId, changeInfo, tab) {
        ...
        if (this.reportSmartScreen) {
            this.reportHelper.sendSmartScreenReport(tabId, url);
        }
    }
}
...
class SortingListBase {
    ...
    onCategorised(categoriesOrUrl, category) {
        ...
        else if (maybe_some(category)) {
            this.onCategorisedCallback({ category, list: this.type, urlSpec: categoriesOrUrl });
        }
    }
}
...
class EndpointSortingList extends SortingListBase {
    ...
    onCategoryResponse(payload) {
        ...
        if (maybe_some(urlSpec)) {
            this.onCategorised(urlSpec, payload.category);
...
class FileUrlSortingList extends SortingListBase {
    ...
    onQueryComplete(url, isTrusted) {
        ...
        if (this.waitingList.has(urlSpec)) {
            this.onCategorised(urlSpec, isTrusted ? IPPageCategory.Allowed : IPPageCategory.Blocked);
    ...
    categorise(url) {
        ...
        if (this.treatFileUrlsAsBlocked || this.sbxSecureBrowserMode) {
            return IPPageCategory.Blocked;
        }
        this.waitingList.add(url.href);
...
class CategoryDetails {
    ...
    equals(other) {
        return maybe_some(other) && this.category === other.category && this.list === other.list;
    }
}
class WaitingItem {
    ...
    update(list, category) {
        this.listResults[list] = new CategoryDetails(category, list);
    }
}
class PhishingCategoryTracker {
    ...
    logCategory(list, urlSpec, category) {
        if (maybe_some(category)) {
            log_log(`${IPSortingList[list]} classified ${urlSpec} as ${IPPageCategory[category]}`);
        }
    }
    ...
    onNewCategoryAvailable(event) {
        ...
        waitingItem.update(event.list, event.category);
    ...
    consultLists(url, callback) {
        ...
        for (const list of this.lists.values()) {
            ...
            if (maybe_some(category)) {
                waitingItem.update(list.type, category);
            }
        }
        const { categoryDetails, final } = waitingItem.getCategory();
...
;// CONCATENATED MODULE: ../lib/common/install-state.ts



var InstallState;
(function (InstallState) {
    InstallState[InstallState["unknown"] = 0] = "unknown";
    InstallState[InstallState["developmentInstall"] = 1] = "developmentInstall";
    InstallState[InstallState["forceInstall"] = 2] = "forceInstall";
    InstallState[InstallState["normalInstall"] = 3] = "normalInstall";
})(InstallState || (InstallState = {}));
class InstallStateManager {
    constructor() {
        this.onStateFetched = new ConditionDispatcher();
        this._state = InstallState.unknown;
        chrome.management.getSelf(info => this.gotSelf(info));
    }
    isForceInstall(info) {
        if (currentBrowser === Browser.firefox) {
            return !info.mayDisable;
        }
        else {
            return info.installType === "admin";
        }
    }
    gotSelf(info) {
        if (info.installType === "development") {
            log_log("The extension is an unpacked developer extension");
            this._state = InstallState.developmentInstall;
        }
        else if (this.isForceInstall(info)) {
            log_log("The extension is force-installed");
            this._state = InstallState.forceInstall;
        }
        else {
            log_log("The extension is not force-installed. Features requiring blocking webRequest will be disabled");
            this._state = InstallState.normalInstall;
        }
        this.onStateFetched.setCondition();
    }
    get state() {
        return this._state;
    }
...
class EventPageListener {
    constructor() {
        const installStateManager = new InstallStateManager();
        ...
        const featureManager = new SBXFeatureManager(configNotifier, errorHandler, cache);
        ...
        const reportHelper = new IdentityProtectionReportHelper(phishingCategoryTracker, phishingNavSequenceDetector, messageSender);
        new ContentScriptController(configNotifier, errorHandler, webRequestListener, hostHelperChannel, linkProtectionService, tabStalker, phishingCategoryTracker, onConfiguredManager, messageSender, reportHelper, badgeManager);
        new BrowserSmartScreenTracker(configNotifier, tabStalker, reportHelper);
...
initChromeRuntime();
const eventPageListener = new EventPageListener();
